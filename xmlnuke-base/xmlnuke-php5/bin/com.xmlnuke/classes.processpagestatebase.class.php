...
abstract class ProcessPageStateBase extends XmlnukeDocumentObject implements IProcessPageState
{
	...
	*/
	public function ProcessPageStateBase($context, $fields, $header, $module, $buttons)
	{
		...
		$this->_lang = LanguageFactory::GetLanguageCollection($context, LanguageFileTypes::OBJECT, basename(__FILE__));
	}
...
//			ProcessPageFields $field
		...
		if (sizeof($nvc)!=0)
		{
			...
			*/
			$p = new XmlnukeSpanCollection();
                        $texto = "";
                        $texto .= '<div id="meusPontosDeVistas"><div style="color:#A92D1E;">Erros(s)</div>';
                        foreach ($nvc as $valor) $texto .= $valor.'<br/>';
                        $texto .= '<br/><a href="javascript:history.go(-1)">Clique aqui para Retornar</a>';
                        $texto .= '</div>';
                        $p->addXmlnukeObject(new XmlNukeText($texto));
