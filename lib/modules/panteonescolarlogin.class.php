 ...
 */
class PanteonEscolarLogin extends PanteonEscolarBasicLogin
{
   ...
   * @var String
   */
  private $_module = "panteonescolar.panteonescolarlogin";
  /**
   * BlockCenter
   *
  ...
  protected function CreateNewUser()
  {
    ...
    $textbox = new XmlInputTextBox("Nome Completo", 'name', $this->_context->ContextValue("name"), 30);
    $textbox->setInputTextBoxType(InputTextBoxType::TEXT);
    $textbox->setDataType(INPUTTYPE::TEXT);
    $textbox->setRequired(true);
    $form->addXmlnukeObject($textbox);

    $textbox = new XmlInputTextBox("E-Mail", 'email', $this->_context->ContextValue("email"), 30);
    $textbox->setInputTextBoxType(InputTextBoxType::TEXT);
    $textbox->setDataType(INPUTTYPE::EMAIL);
    $textbox->setRequired(true);
    $form->addXmlnukeObject($textbox);
    ...
    $textbox = new XmlEasyList(EasyListType::SELECTLIST, "id_instituicao", "Instituição", $instituicoes, $this->_context->ContextValue("id_instituicao"));
    $textbox->setRequired(true);
    $form->addXmlnukeObject($textbox);

    $captcha = new XmlInputImageValidate('Verificação');
    $form->addXmlnukeObject($captcha);

    $arrayTermo = array("" => "-- Selecione  --", 1 => "Aceito");


    if($this->_context->ContextValue("xsl") == "login")
    {
      $termoDeUso = new XmlEasyList(EasyListType::SELECTLIST, "termo_de_uso", '<a href="xmlnuke.php?module=panteonescolar.termodeuso&amp;site=PanteonEscolar&amp;xsl=login&amp;lang=pt-br">Termo De Uso</a>', $arrayTermo, $this->_context->ContextValue("termo_de_uso"));
    }

    else
    {
      $termoDeUso = new XmlEasyList(EasyListType::SELECTLIST, "termo_de_uso", '<a class="lista_direita_detalhe" href="xmlnuke.php?module=panteonescolar.termodeuso&amp;site=PanteonEscolar&amp;xsl=ver&amp;lang=pt-br">Termo De Uso</a>', $arrayTermo, $this->_context->ContextValue("termo_de_uso"));
    }


    $termoDeUso->setRequired(true);
    $form->addXmlnukeObject($termoDeUso);

    /*
    if ($this->_context->ContextValue("xsl") == "login")
        $termo = new XmlInputCheck('Concordo com o <a href="xmlnuke.php?module=panteonescolar.termodeuso&amp;site=PanteonEscolar&amp;xsl=login&amp;lang=pt-br">Termo de Uso?</a>', 'termo_uso_usuario', '1');
    else
        $termo = new XmlInputCheck('Concordo com o <a class="lista_direita_detalhe" href="xmlnuke.php?module=panteonescolar.termodeuso&amp;site=PanteonEscolar&amp;xsl=ver&amp;lang=pt-br">Termo de Uso?</a>', 'termo_uso_usuario', '1');
     *
     */
