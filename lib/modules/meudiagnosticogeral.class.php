...
class MeuDiagnosticoGeral extends PanteonEscolarBaseModule
{

  public function CreatePage() {
    $myWords = $this->WordCollection();

    $titulo = "Meu Diagnóstico Geral";
    $voltar['titulo'] = "Meus Diagnósticos";
    $voltar['url'] = "/meusdiagnosticos";

    $this->defaultXmlnukeDocument = PanteonEscolarBaseModule::definirTitulo($titulo, $this->_context->getCookie("nome_tema_panteon_definido"), $voltar);
    ...
    $blockHead->addXmlnukeObject(new MeuDiagnosticoGeralDBXML($this->_context, "menuHead"));

    $this->defaultXmlnukeDocument->addXmlnukeObject($blockInfo);
    $blockInfo->addXmlnukeObject(new MeuDiagnosticoGeralDBXML($this->_context, "listarDireita"));

    $this->defaultXmlnukeDocument->addXmlnukeObject($blockMenu);
    $blockMenu->addXmlnukeObject(new MeuDiagnosticoGeralDBXML($this->_context, "menu"));

    $this->defaultXmlnukeDocument->addXmlnukeObject($blockCenter);
    $blockCenter->addXmlnukeObject(new MeuDiagnosticoGeralDBXML($this->_context, "processPageField"));
