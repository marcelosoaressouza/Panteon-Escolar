...
class PanteonEscolarBaseModule extends BaseModule
{
  ...
  public function preencherBarra($node, $itDB, $titulo, $conteudo = "", $autor = " ", $link = " ", $id = "") {
    ...
    while($itDB->hasNext()) {
      ...
      if(($link !="") && ($id !="")) $linkFinal = '<div id="link_barra_direita">'. '<a class="lista_direita_detalhe" ' . 'title="Detalhes '.$sr->getField($autor) .'" href="'. $link . $sr->getField($id) . '"> Leia Mais </a></div>';
      ...
      if($titulo != "") XmlUtil::AddAttribute($body, "titulo", "<b>".$sr->getField($titulo)."</b><br/>");

      XmlUtil::AddAttribute($body, "conteudo", $texto);
      XmlUtil::AddAttribute($body, "autor", $sr->getField($autor));

      if(($link !="") && ($id !="")) XmlUtil::AddAttribute($body, "link", $linkFinal);
  ...
  public function preencherMenuConfig($ativo = "", $permissao = "") {
    ...
    if($permissao == 'ADMINISTRADOR') $menu[PanteonEscolarMenu::TipoMidia] = "configtipomidia";
...
class PanteonEscolarMyProcess extends ProcessPageStateDB
{
  public function renderField($field, $curValue) {
    if($field->fieldXmlInput == XmlInputObjectType::CUSTOM) return new XmlInputFile($field->fieldCaption);
    else return parent::renderField($field, $curValue);
  }
}
