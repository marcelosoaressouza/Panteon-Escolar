...
class UsuarioXGrupoDBXML extends XmlnukeCollection implements IXmlnukeDocumentObject
{
  ...
  public function criarProcessPageFields($id_usuario = "", $permissao = "", $id_grupo = "")
  {
    ...
    if($id_grupo != "")
    {
      ...
      $it = $db->obterTodosOsUsuariosQueNaoEstaoNoGrupoPorIDTemaPanteonEPorIDGrupo($id_tema_panteon, $id_grupo);
      ...
      {
        $db = new UsuarioXTemaPanteonDB($this->_context);
        $it = $db->obterTodosOsUsuariosQueEstaoNoGrupoPorIDTemaPanteonEPorIDGrupo($id_tema_panteon, $id_grupo);
        $arrayUsuario = PanteonEscolarBaseDBAccess::getArrayFromIterator($it, "id_usuario", "nome_completo_usuario");
      }
    }
    ...
    if($id_grupo != "")
    {
      $field = ProcessPageFields::FactoryMinimal("id_usuario", "Usuários", 30, true, true);
    }
    ...
    if($id_grupo   != "")
    {
      $processpage->setFilter(" id_grupo   = " . $id_grupo);
    }
