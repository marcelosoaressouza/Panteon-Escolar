...
class MensagemForumDBXML extends XmlnukeCollection implements IXmlnukeDocumentObject
{
  ...
  public function criarProcessPageFields($id_forum = "", $id_usuario = "", $permissao = "", $id_mensagem_original = NULL, $respostas = false, $id_usuario_por = "")
  {
    ...
    if($id_mensagem_original != NULL)
    {
      ...
      //$nomeMensagemOriginalForum = $dbUsuario->obterPorId($id_mensagem_original)->getNomeMensagemForum();
      $autorMensagemOriginalForum = $dbUsuario->obterPorId($id_mensagem_original)->getNomeCompletoUsuario();
    }
    ...
    if($id_mensagem_original != NULL)
    {
      ...
      /*
      $field = ProcessPageFields::FactoryMinimal("nome_mensagem_original", "Título Mensagem Original", 30, true, false);
      $field->fieldXmlInput = XmlInputObjectType::TEXTBOX;
      $field->editable = false;
      $field->defaultValue = $nomeMensagemOriginalForum;
      $fieldList->addProcessPageField($field);

      $field = ProcessPageFields::FactoryMinimal("nome_autor_mensagem_original", "Autor Mensagem Original", 30, true, false);
      $field->fieldXmlInput = XmlInputObjectType::TEXTBOX;
      $field->editable = false;
      ...
      if($respostas == true) {
        ...
        if($itMensagemForum->hasNext()) {
          ...
          while($itMensagemForum->hasNext()) {
            ...
            if($itMensagemForum->hasNext()) $filtro .= " OR ";
