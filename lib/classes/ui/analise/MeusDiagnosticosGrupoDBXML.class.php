...
class MeusDiagnosticosGrupoDBXML extends XmlnukeCollection implements IXmlnukeDocumentObject
{
  ...
  public function generateObject($current) {
    ...
    if($this->_opcao == "processPageField") {
      ...
      $dbxml = new DiagnosticoGrupoDBXML($this->_context, "meusdiagnosticosgrupo", "Meus Diagnósticos do Grupo");
      ...
      if($pagina->getAllRecords()->Count() > 0) {
        ...
        if(($this->_context->ContextValue("acao") == "ppnew") || ($this->_context->ContextValue("chamada") == 1)) {
          ...
          $span1->addXmlnukeObject($this->filtro());
          $msg = '<div id="meusPontosDeVistas"> Nenhum Diagnóstico do Grupo encontrado, cadastre o primeiro agora. <br/><a href="'.PanteonEscolarBaseModule::curPageURL().'&acao=ppnew">Clicando Aqui</a></div>';
          if($id_grupo == 65535) {
  ...
  public function filtro() {
    ...
    $formPost = "module:panteonescolar.meusdiagnosticosgrupo";
    $form = new XmlFormCollection($this->_context, $formPost, "Meus Diagnósticos de Grupo");
  ...
  public function filtroGrupo() {
    ...
    $listaGrupo[""] = "Meus Diagnósticos de Grupo";
  ...
  public function MeusDiagnosticosGrupoDBXML($context, $opcao) {
